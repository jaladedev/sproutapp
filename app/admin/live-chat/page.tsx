...
export default function AdminWithdrawalsPage() {
  ...
  const handleApprove = async (withdrawal: Withdrawal) => {
    if (!(await confirm({ message: `Approve ₦${(withdrawal.amount_kobo / 100).toLocaleString()} withdrawal for ${withdrawal.user?.name}? This will initiate a Paystack transfer immediately.` }))) return;
  ...
  const handleApproveAll = async () => {
    ...
    if (!(await confirm({ message: `Approve all ${pendingCount} pending withdrawal${pendingCount !== 1 ? "s" : ""}? This will initiate Paystack transfers for each.` }))) return;
